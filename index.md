...
Redis is an open source, in-memory, key-value store. Values in Redis can be simple strings, hashes, lists, and sets or powerful bitmaps, hyperloglogs, and geospatial indexes. Redis is ideal as an application cache or quick response data store.

The latest and greatest Redis service is [{{site.data.keyword.databases-for-redis_full}}](/docs/databases-for-redis?topic=databases-for-redis-getting-started)! You can still create and use {{site.data.keyword.composeForRedis}} deployments, but please consider moving to {{site.data.keyword.databases-for-redis}}. If you already have a {{site.data.keyword.composeForRedis}} deployment, consider testing out [A How-To for Migrating Redis to IBM Cloud Databases for Redis](https://www.ibm.com/cloud/blog/a-how-to-for-migrating-redis-to-ibm-cloud-databases-for-redis).
{: .important}
